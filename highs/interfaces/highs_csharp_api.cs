...
#region "Callbacks as events"
                                  ...
                                  [In, Out] ref HighsCallbackDataIn cbDataIn, IntPtr cbUserData)
    {
        switch (cbType)
        {
            ...
            case HighsCallbackType.kCallbackMipInterrupt:
                ...
                _innerInterrupt?.Invoke(this, interruptArgs);
    ...
    public event EventHandler<MipEventArgs> MipStatusReported
    {
        add
        {
            lock (_mipLoggingLockObject)
            {
                if (_innerMipLogging == null)
                    this.startCallback(HighsCallbackType.kCallbackMipLogging);
        ...
        remove
        {
            lock (_mipLoggingLockObject)
            {
                _innerMipLogging -= value;
                if (_innerMipLogging == null)
                    this.stopCallback(HighsCallbackType.kCallbackMipLogging);
            }
        }
    }
    ...
    private EventHandler<InterruptCheckEventArgs> _innerInterrupt;
    /// <summary>Occurs when the solver checks whether stopping criteria have been satisfied</summary>
    /// <remarks>If the client wishes to terminate the solve, set the event's user_interrupt to true</remarks>
    public event EventHandler<InterruptCheckEventArgs> InterruptCheck
    {
        add
        {
            lock (_mipInterruptLockObject)
            {
                if (_innerInterrupt == null)
                    this.startCallback(HighsCallbackType.kCallbackMipInterrupt);
                    _innerInterrupt += value;
            }
        }
        remove
        {
    ...
    public class InterruptCheckEventArgs : MipEventArgs
    {
        internal InterruptCheckEventArgs(HighsCallbackDataOut data) :
            base(data)
        {}

        /// <summary>Whether to interrupt the solver operation currently </summary>
