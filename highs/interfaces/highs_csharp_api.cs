...
#region "Callbacks as events"
    ...
    private void callbackFunction(HighsCallbackType cbType, IntPtr messagePtr, [In] ref HighsCallbackDataOut cbDataOut, [In, Out] ref HighsCallbackDataIn cbDataIn, IntPtr cbUserData)
    {
        switch (cbType)
        {
            case HighsCallbackType.kCallbackLogging:
                // We receive the message as an IntPtr instead of a string so that the marshaller doesn't attempt to free the C string.
            ...
            case HighsCallbackType.kCallbackMipInterrupt:
                var mipEventData = new MipEventArgs(cbDataOut);
                var evnt = cbType == HighsCallbackType.kCallbackMipImprovingSolution ? _innerMipImproving
                        : cbType == HighsCallbackType.kCallbackMipInterrupt ? _innerMipInterrupt
                        : null;
                evnt?.Invoke(this, mipEventData);
                if (mipEventData.user_interrupt)
    ...
    public event EventHandler<LoggingEventArgs> LogReceived
    {
        add
        {
            lock (_logReceivedLockObject)
            {
                ...
                if (_innerLogReceived == null)
                    this.startCallback(HighsCallbackType.kCallbackLogging);
        ...
        remove
        {
            lock (_logReceivedLockObject)
            {
                ...
                if (_innerLogReceived == null)
                    this.stopCallback(HighsCallbackType.kCallbackLogging);
            }
        }
    }
    ...
    public event EventHandler<MipEventArgs> MipImprovingSolutionFound
    {
        add
        {
            lock (_mipImprovingLockObject)
            {
                if (_innerMipImproving == null)
                    this.startCallback(HighsCallbackType.kCallbackMipImprovingSolution);
        ...
        remove
        {
            lock (_mipImprovingLockObject)
            {
                _innerMipImproving -= value;
                if (_innerMipImproving == null)
                    this.stopCallback(HighsCallbackType.kCallbackMipImprovingSolution);
            }
        }
    }
    ...
    private EventHandler<MipEventArgs> _innerMipInterrupt;
    /// <summary>Occurs when the MIP solver checks whether stopping criteria have been satisfied</summary>
    /// <remarks>If the client wishes to terminate the solve, set the event's user_interrupt to true</remarks>
    public event EventHandler<MipEventArgs> MipInterruptCheck
    {
        add
        {
            lock (_mipInterruptLockObject)
            {
                if (_innerMipInterrupt == null)
                    this.startCallback(HighsCallbackType.kCallbackMipInterrupt);
        ...
        remove
        {
            lock (_mipInterruptLockObject)
            {
                _innerMipInterrupt -= value;
                if (_innerMipInterrupt == null)
                    this.stopCallback(HighsCallbackType.kCallbackMipInterrupt);
            }
        }
    }
#endregion
...
/// <summary>Data for logging events</summary>
public class LoggingEventArgs : EventArgs
{
    /// <summary>The type/level of log message</summary>
    public HighsLogType log_type { get; }
...
/// <summary>Data for MIP events</summary>
public class MipEventArgs : EventArgs
{
    /// <summary>The execution time in seconds</summary>
    public double running_time { get; }
    /// <summary>The objective function value of the best integer feasible solution found so far</summary>
    public double objective_function_value { get; }
    /// <summary>The number of MIP nodes explored so far</summary>
    public long mip_node_count { get; }
    /// <summary>The primal bound</summary>
    public double mip_primal_bound { get; }
    /// <summary>The dual bound</summary>
    public double mip_dual_bound { get; }
    /// <summary>The relative difference between the primal and dual bounds</summary>
    public double mip_gap { get; }
    /// <summary>Set to true to interrupt the current solve operation</summary>
    public bool user_interrupt { get; set; }

    internal MipEventArgs(HighsCallbackDataOut data)
    {
        this.running_time = data.running_time;
        this.objective_function_value = data.objective_function_value;
        this.mip_node_count = data.mip_node_count;
        this.mip_primal_bound = data.mip_primal_bound;
        this.mip_dual_bound = data.mip_dual_bound;
        this.mip_gap = data.mip_gap;
